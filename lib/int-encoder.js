...
(function() {
	module.exports = (function(alphabet) {
		...
		function encode(value, base) {
			...
			return encodeBignum(value);
		};

		function encodeBignum(value) {
			var result = '';
			if (value.eq(0)) {
				return alphabet[0];
			}
		...
		function decode(str, base) {
			...
			return decodeBignum(str).toString(base);
		};
		...
		methods = {
			encode: encode,
			decode: decode,
			encodeBignum: encodeBignum,
			decodeBignum: decodeBignum,
			alphabet: alpha
		};
